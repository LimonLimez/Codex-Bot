...
async function main() {
  ...
  if (!Number.isInteger(width) || !Number.isInteger(height) || width < 640 || height < 480
    || !new Set(["medium", "max", "ultra"]).has(effort)) {
  ...
  await window.webContents.executeJavaScript(`(() => {
    ...
    const positions = { medium: 1, max: 4, ultra: 5 };
    if (!slider) throw new Error("Reasoning control did not mount.");
    slider.value = String(positions[${JSON.stringify(effort)}]);
    slider.dispatchEvent(new Event("input", { bubbles: true }));
  })()`);
  await new Promise((resolve) => setTimeout(resolve, effort === "ultra" ? 700 : 150));
