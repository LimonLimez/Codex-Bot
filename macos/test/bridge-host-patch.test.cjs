...
test("the Grok 0.20 host inference path is exclusively routed through the Codex bridge", () => {
  ...
  assert.match(patched, /ye=\{conversationId:t\.getConversationId\(\),modelId:t\.subagentModelId/);
  assert.match(patched, /\{conversationId:t\.getConversationId\(\),modelId:dXt,isSummarizationSession:!0/);
