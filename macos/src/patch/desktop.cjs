...
const LOCAL_FRAME_PRELOAD_PATCH = `const __openbotLocalDesktopListener=(channel,callback,label)=>{if(typeof callback!=="function")throw new TypeError("Local Desktop "+label+" listener must be a function.");const listener=(_event,value)=>callback(value);s.ipcRenderer.on(channel,listener);return()=>s.ipcRenderer.removeListener(channel,listener)};const __openbotLocalDesktop=Object.freeze({presentation:value=>s.ipcRenderer.invoke("openbot-local-frame:presentation",value),select:value=>s.ipcRenderer.invoke("openbot-local-frame:select",value),retry:value=>s.ipcRenderer.invoke("openbot-local-frame:retry",value),clear:value=>s.ipcRenderer.invoke("openbot-local-frame:clear",value),navigate:value=>s.ipcRenderer.invoke("openbot-local-frame:navigate",value),goBack:value=>s.ipcRenderer.invoke("openbot-local-frame:go-back",value),goForward:value=>s.ipcRenderer.invoke("openbot-local-frame:go-forward",value),reload:value=>s.ipcRenderer.invoke("openbot-local-frame:reload",value),acquireControl:value=>s.ipcRenderer.invoke("openbot-local-frame:acquire-control",value),releaseControl:value=>s.ipcRenderer.invoke("openbot-local-frame:release-control",value),sendInput:value=>s.ipcRenderer.invoke("openbot-local-frame:send-input",value),onFrame:callback=>__openbotLocalDesktopListener("openbot-local-frame:frame",callback,"frame"),onStatus:callback=>__openbotLocalDesktopListener("openbot-local-frame:status",callback,"status"),onNavigation:callback=>__openbotLocalDesktopListener("openbot-local-frame:navigation",callback,"navigation")});s.contextBridge.exposeInMainWorld("openbotLocalDesktop",__openbotLocalDesktop);`;
