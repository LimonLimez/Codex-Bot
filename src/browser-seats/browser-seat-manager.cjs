...
const { publicOriginForLog, redactError, redactLogDetails } = require(
  path.join(__dirname, "..", "codex-connection.cjs"),
);
const { createPublicWebProxy } = require(
  path.join(__dirname, "public-web-proxy.cjs"),
);
const { BrowserActionApprovalCoordinator } = require(
  path.join(__dirname, "browser-action-approval.cjs"),
);
const { BrowserControlLeaseCoordinator } = require(
  path.join(__dirname, "browser-control-lease.cjs"),
);
const { CHROMIUM_HARDENING_ARGS, installBrowserPageHardening } = require(
  path.join(__dirname, "browser-page-hardening.cjs"),
);
...
const LEGACY_CHROME_SESSION_FILES = Object.freeze([
  "Current Session",
  "Current Tabs",
  "Last Session",
  "Last Tabs",
...
function isBlockedNavigationHostname(value) {
  ...
  return mapped
    ...
    : BLOCKED_NETWORKS.check(hostname, "ipv6");
}
...
function publicWebUrl(value) {
  ...
  try {
    ...
    throw new Error(
      "Isolated browser seats can open only public HTTP(S) pages.",
    );
  }
  if (
    (parsed.protocol !== "http:" && parsed.protocol !== "https:") ||
    !parsed.hostname ||
    isBlockedNavigationHostname(parsed.hostname)
  ) {
    throw new Error(
      "Isolated browser seats can open only public HTTP(S) pages.",
    );
...
function assertSafePageForCapture(page) {
  if (!isAllowedLoadedPageUrl(page.url())) {
    throw new Error(
      "Isolated browser navigation was blocked before page capture.",
    );
  }
}
...
function profileIdFor(key) {
  return crypto
    .createHash("sha256")
    .update(String(key || "default-seat"))
    .digest("hex")
    .slice(0, 20);
}
...
function normalizeSessionState(value) {
  ...
  for (
    let originalIndex = 0;
    originalIndex < value.tabs.length && entries.length < MAX_RESTORED_TABS;
    originalIndex += 1
  ) {
  ...
  let activeIndex = entries.findIndex(
    (entry) => entry.originalIndex === requestedIndex,
  );
  if (activeIndex < 0) {
    activeIndex = 0;
    for (
      let index = 0;
      index < entries.length && entries[index].originalIndex <= requestedIndex;
      index += 1
    )
      activeIndex = index;
  }
...
function readSessionState(key) {
  try {
    return normalizeSessionState(
      JSON.parse(fs.readFileSync(sessionStatePathFor(key), "utf8")),
    );
  } catch {
    return null;
  }
...
function preparePersistentProfileForSafeLaunch(profileDir) {
  const chromeProfileDir = path.join(profileDir, CHROME_PROFILE_NAME);
  const preferencesPath = path.join(chromeProfileDir, "Preferences");
  let preferences = {};
  if (fs.existsSync(preferencesPath)) {
    try {
      preferences = objectRecord(
        JSON.parse(fs.readFileSync(preferencesPath, "utf8")),
      );
    } catch (error) {
      throw new Error(
        `The isolated browser profile preferences are unreadable; refusing an unsafe startup: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }

  preferences.profile = objectRecord(preferences.profile);
  preferences.profile.exit_type = "Normal";
  preferences.profile.exited_cleanly = true;
  preferences.session = objectRecord(preferences.session);
  preferences.session.restore_on_startup = 5;
  preferences.session.startup_urls = [];
  atomicWriteJson(preferencesPath, preferences);

  const sessionDirectory = path.join(chromeProfileDir, "Sessions");
  if (fs.existsSync(sessionDirectory)) {
    for (const entry of fs.readdirSync(sessionDirectory, {
      withFileTypes: true,
    })) {
      if (!/^(?:Session|Tabs)_/.test(entry.name)) continue;
      if (!entry.isFile() && !entry.isSymbolicLink()) continue;
      fs.rmSync(path.join(sessionDirectory, entry.name), { force: true });
    }
  }
  for (const name of LEGACY_CHROME_SESSION_FILES) {
    const candidate = path.join(chromeProfileDir, name);
    try {
...
async function quarantinePersistentStartupPages(
  context,
  preferredAnchor = null,
) {
  let anchor =
    preferredAnchor && !preferredAnchor.isClosed() ? preferredAnchor : null;
  for (let pass = 0; pass < 4; pass += 1) {
    const pages = context.pages().filter((page) => !page.isClosed());
    if (!anchor || anchor.isClosed()) anchor = pages[0] || null;
    if (anchor && anchor.url() !== "about:blank") {
      await anchor.goto("about:blank", { waitUntil: "commit", timeout: 10000 });
    }
    await Promise.all(
      pages
    ...
    if (context.pages().every((page) => page.isClosed() || page === anchor))
      return anchor;
  }
  if (context.pages().some((page) => !page.isClosed() && page !== anchor)) {
    throw new Error(
      "Chrome created an extra page while its persistent profile was quarantined.",
    );
  }
  return anchor;
}
  ...
  page.on("framenavigated", (frame) => {
    if (frame === page.mainFrame()) {
      ...
      if (!isAllowedLoadedPageUrl(frame.url())) {
        log("navigation_blocked_after_commit", {
          profileId: seat.profileId,
          target: navigationLogTarget(frame.url()),
        });
...
function resolveChrome() {
  ...
  if (!found)
    throw new Error(
      "No compatible Chrome or Edge executable was found for isolated browser seats.",
    );
  return found;
}
...
async function withAllocationLock(fn) {
  ...
  let release;
  allocationTail = new Promise((resolve) => {
    release = resolve;
  });
  await previous;
  try {
...
async function currentPage(seat) {
  if (
    seat.page &&
    !seat.page.isClosed() &&
    !isAllowedLoadedPageUrl(seat.page.url())
  ) {
    const unsafePage = seat.page;
    seat.page = null;
    await unsafePage.close().catch(() => {});
  }
  const pages = seat.context
    .pages()
    .filter((page) => !page.isClosed() && isAllowedLoadedPageUrl(page.url()));
...
async function launchSeat(key) {
  ...
  let seat = null;
  try {
    let quarantineAnchorPage =
      context.pages().find((page) => !page.isClosed()) || null;
    const quarantineStartupPage = (page) => {
      if (!quarantineAnchorPage || quarantineAnchorPage.isClosed()) {
        quarantineAnchorPage = page;
        if (page.url() !== "about:blank")
          void page
            .goto("about:blank", { waitUntil: "commit", timeout: 10000 })
            .catch(() => {});
        return;
      }
      if (page !== quarantineAnchorPage)
        void page.close({ runBeforeUnload: false }).catch(() => {});
    };
    context.on("page", quarantineStartupPage);
    quarantineAnchorPage = await quarantinePersistentStartupPages(
      context,
      quarantineAnchorPage,
    );
    ...
    await context.route("**/*", async (route, request) => {
      ...
      if (!allowed)
        log("navigation_blocked", {
          profileId,
          target: navigationLogTarget(request.url()),
        });
    });
    seat = {
      key,
    ...
    context.off("page", quarantineStartupPage);
    if (quarantineAnchorPage && !quarantineAnchorPage.isClosed()) {
      seat.page = quarantineAnchorPage;
      attachPage(seat, quarantineAnchorPage);
    }
    context.on("close", () => {
    ...
    if (restoredSession != null) {
      // Startup pages were quarantined above; rebuild only the canonical,
      // sanitized app snapshot after routes and init scripts are installed.
      await Promise.all(
        context
          .pages()
      ...
      const results = await Promise.allSettled(
        restoredPages.map((page, index) =>
          page.goto(publicWebUrl(restoredSession.tabs[index]), {
      ...
      seat.page =
        restoredPages[
          Math.min(restoredSession.activeIndex, restoredPages.length - 1)
        ];
      seat.lastPageInfo = await classifyPage(seat.page).catch(() => ({
        state: "error",
        title: "",
        bodyPreview: "",
      }));
      log("session_restored", {
        profileId,
        tabCount: restoredPages.length,
        failedCount,
        target: navigationLogTarget(seat.page.url()),
      });
    } else if (firstPage.url() === "about:blank") {
      try {
        await firstPage.goto(publicWebUrl(DEFAULT_START_URL), {
          waitUntil: "domcontentloaded",
          timeout: 45000,
        });
    ...
    if (seat) seat.closing = true;
    if (seat && activeSeats.get(key) === seat) activeSeats.delete(key);
    try {
      await context.close();
    } catch {}
    try {
      await publicWebProxy.close();
    } catch {}
    throw error;
  }
}
...
async function ensureSeat(key) {
  return await withAllocationLock(async () => {
    ...
    if (activeSeats.size >= MAX_ACTIVE) {
      const idle = [...activeSeats.values()]
        .filter((seat) => seat.pending === 0)
        .sort((a, b) => a.lastUsed - b.lastUsed)[0];
      if (!idle)
        throw new Error(
          `All ${MAX_ACTIVE} isolated browser seats are busy. Retry when one finishes.`,
        );
...
async function navigateAddress(seat, page) {
  ...
  await page.goto(publicWebUrl(target), {
    waitUntil: "domcontentloaded",
    timeout: 45000,
  });
}
    ...
    return await page.evaluate(
      ({ x, y, useActive }) => {
        ...
        return {
          ...
          formAction:
            target.formAction || target.closest?.("form")?.action || "",
          formMethod:
            target.formMethod || target.closest?.("form")?.method || "",
        };
      },
      {
...
function seatAddressAction(action) {
  return (
    String(action?.kind || "").toLowerCase() === "key" &&
    /^(?:CTRL|CONTROL)\+L$/i.test(String(action?.key || ""))
  );
}
...
async function approvalContextForActions(page, actions, options = {}) {
  ...
  for (const action of actions || []) {
    ...
    const hasPoint = Boolean(
      action?.coordinate ||
      (Array.isArray(action?.path) && action.path.length > 0),
    );
    ...
    if (!addressMode && (kind === "type" || kind === "key") && lastPointTarget)
      target = lastPointTarget;
    contexts.push({
      surface: addressMode ? "address" : undefined,
      target,
    });
    if (!addressMode && hasPoint && Object.keys(target || {}).length > 0)
      lastPointTarget = target;
    else if (!addressMode && kind !== "type" && kind !== "key")
      lastPointTarget = null;
    if (
      addressMode &&
      String(action?.kind || "").toLowerCase() === "key" &&
      /^(ENTER|RETURN|ESC|ESCAPE)$/i.test(String(action?.key || ""))
    ) {
      addressMode = false;
      lastPointTarget = null;
    }
...
async function executeAction(seat, page, action) {
  ...
  switch (kind) {
    case "click": {
      seat.address = null;
      seat.cursor = {
        x: Number(action.coordinate?.x || 0),
        y: Number(action.coordinate?.y || 0),
      };
    ...
    case "type": {
      ...
      if (seat.address) {
        seat.address.text = seat.address.selected
          ? text
          : seat.address.text + text;
    ...
    case "key": {
      ...
      if (isChord(key, "CTRL+L")) {
        seat.address = {
          text: page.url() === "about:blank" ? "" : page.url(),
          selected: true,
        };
    ...
    case "scroll": {
      seat.address = null;
      seat.cursor = {
        x: Number(action.coordinate?.x ?? seat.cursor.x),
        y: Number(action.coordinate?.y ?? seat.cursor.y),
      };
      await page.mouse.move(seat.cursor.x, seat.cursor.y);
    ...
    case "wait":
      await page.waitForTimeout(
        Math.max(0, Math.min(30000, Number(action.durationMs || 1000))),
      );
      break;
    case "screenshot":
      break;
    default:
      throw new Error(
        `Unsupported isolated-browser computer action: ${String(kind || "unknown")}`,
      );
  }
}
...
async function classifyPage(page) {
  ...
  if (/^(chrome-error|edge-error):/i.test(url))
    return { state: "error", title, bodyPreview: "" };
...
async function executeSeatActions(key, actions, options = {}) {
  ...
  if (actor === "agent")
    browserControls.assertAgentAllowed(String(key || "default-seat"));
    ...
    try {
      for (let index = 0; index < actionSnapshot.length; index += 1) {
        ...
        for (let attempt = 0; attempt < 4 && !stable; attempt += 1) {
          ...
          const approvalContext = await approvalContextForActions(
            page,
            [action],
            contextOptions,
          );
          ...
          const currentContext = await approvalContextForActions(
            current,
            [action],
            {
              addressMode: Boolean(seat.address),
          ...
          if (stable) validatedPage = current;
        }
        if (!stable)
          throw new Error(
            "The page changed while this browser action was awaiting approval. Please retry it on the current page.",
          );
        browserControls.assertAgentAllowed(seat.key);
        page = validatedPage;
        await executeAction(seat, page, action);
      ...
      if (actor === "user") {
        ...
        return {
          ...
          pageState:
            seat.lastPageInfo?.state ||
            (page.url() === "about:blank" ? "blank" : "loaded"),
          bodyPreview: "",
          profileId: seat.profileId,
      ...
      log("actions_complete", {
        profileId: seat.profileId,
        actionCount: actionSnapshot.length,
        target: navigationLogTarget(result.url),
        title: result.title,
...
function decidePendingApproval(key, decision, binding) {
  ...
  if (seatKey !== String(binding?.seatId || ""))
    throw new Error("Browser approval does not match this employee.");
  if (decision === "allow-once") return trustedUserApprover.allowOnce(binding);
  if (decision === "allow-site")
    return trustedUserApprover.allowSiteLease(binding);
...
async function closeAllSeats() {
  await withAllocationLock(async () => {
    await Promise.all(
      [...activeSeats.values()].map((seat) => closeSeat(seat, "shutdown")),
    );
  });
}
...
module.exports = {
  ...
  preparePersistentProfileForSafeLaunch,
  quarantinePersistentStartupPages,
  publicWebUrl,
