...
class BrowserControlLeaseCoordinator {
  ...
  constructor({
    now = Date.now,
    ttlMs = DEFAULT_CONTROL_TTL_MS,
    onAcquire = () => {},
  } = {}) {
    if (typeof now !== "function" || typeof onAcquire !== "function")
      throw new TypeError("Control lease callbacks must be functions.");
    const requested = Number(ttlMs);
    if (!Number.isFinite(requested) || requested <= 0)
      throw new RangeError("Control lease expiry must be positive.");
    this.#now = now;
  ...
  acquire(seatId, ownerId) {
    ...
    if (existing && existing.ownerId !== owner)
      throw new Error("Another trusted view already controls this employee.");
  ...
  heartbeat(seatId, ownerId) {
    ...
    if (!existing || existing.ownerId !== owner)
      throw new Error(
        "This trusted view does not own the browser control lease.",
      );
    const lease = Object.freeze({
      seatId: seat,
      ownerId: owner,
  ...
  status(seatId) {
    ...
    return lease
      ? Object.freeze({ controlled: true, expiresAt: lease.expiresAt })
      : Object.freeze({ controlled: false, expiresAt: null });
  }

  assertAgentAllowed(seatId) {
    if (this.status(seatId).controlled)
      throw new Error(
        "The user currently has direct control of this employee's browser.",
      );
    return true;
  }
  ...
  authorizeUser(seatId, ownerId) {
    ...
    if (!lease || lease.ownerId !== owner)
      throw new Error("Take control before sending browser input.");
    return true;
  }

  #seat(value) {
    const seat = String(value || "").trim();
    if (!seat || seat.length > 200)
      throw new TypeError("A valid employee seat key is required.");
    return seat;
  }
  ...
  #owner(value) {
    ...
    if (!/^[A-Za-z0-9_-]{16,128}$/.test(owner))
      throw new TypeError(
        "A valid trusted-view control identifier is required.",
      );
    return owner;
  }
