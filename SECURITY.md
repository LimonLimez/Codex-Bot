...
Security fixes are provided for the latest tagged Open Bot release and the exact Grok Bot version and archive hash listed in the README.
...
- Private browser seats remain the default. Model requests use exactly the provider selected by the Windows user through CLIProxyAPI—or their optional direct OpenAI API key—and fail closed instead of falling back to a different provider.
- CLIProxy connections are limited to the reviewed OpenAI Codex, Anthropic Claude, Google Antigravity, Moonshot Kimi, xAI, and Google Vertex flows exposed by the pinned CLIProxyAPI build. Authorization links are validated against exact HTTPS provider origins before the renderer can open them. Vertex imports require a service-account JSON shape, use a private bounded temporary file, and delete that upload after the local importer finishes.
...
- Before any vendor files are copied or executed by the Open Bot patch step, Setup verifies every file in the supported Grok Bot 0.18.0 tree against the reviewed 657-file manifest and checks the signed executable identity. Unknown, incomplete, modified, extra-file, and reparse-point trees are rejected.
...
- The separate vendor install is not part of the Open Bot rollback boundary. If the outer setup is canceled or fails after vendor installation, Grok Bot remains installed; the Open Bot uninstaller does not remove it.
