...
function createDevelopmentInstallerFixture(t) {
  ...
  const installerName = `OpenBot-Setup-0.1.6-DEVELOPMENT-20260814T000000000Z-${revision}.exe`;
...
[assembly: AssemblyInformationalVersion("0.1.6 DEVELOPMENT TEST BUILD")]
...
test("generation fails closed on an unreviewed hash, non-development name, or reused evidence root", (t) => {
  ...
  const releaseName = "OpenBot-Setup-0.1.6.exe";
  ...
  const invoke = () =>
    run(
      ...
      [
        ...
        "Bypass",
        "-Command",
        String.raw`. $env:CODEX_BOT_RUNNER -Scenario Silent -ArtifactDirectory fixture -HarnessDirectory fixture -EvidenceDirectory fixture -ExpectedSha256 $('0' * 64) -ExpectedInstallerName fixture -ExpectedVersion 0.1.6 -ExpectedRevision 14fcf819cd7a -ExpectedBrandedExecutableSha256 $('1' * 64) -ExpectedPatchInputsSha256 $('2' * 64) -ExpectedVendorManifestSha256 $('3' * 64)
