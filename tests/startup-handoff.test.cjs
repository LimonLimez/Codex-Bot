...
test("installation registers the watchdog but leaves first-run startup to the launcher", () => {
  ...
  assert.match(enableAlwaysOn, /Register-ScheduledTask -TaskName 'Codex Bot Bridge'/);
  assert.doesNotMatch(enableAlwaysOn, /Start-ScheduledTask/);

  const desktopStart = launcher.indexOf("Start-Process -FilePath $portable", launcher.indexOf("if ($DebugRenderer)"));
  const watchdogStart = launcher.indexOf("Start-ScheduledTask -TaskName 'Codex Bot Bridge'");
  assert.ok(desktopStart >= 0, "launcher must start the desktop");
  assert.ok(watchdogStart > desktopStart, "watchdog handoff must happen after the desktop start");
});
...
test("scheduled and interactive launches use the windowless system script host", () => {
  ...
  assert.match(enableAlwaysOn, /\$windowsScriptHost = Join-Path \$env:SystemRoot 'System32\\wscript\.exe'/);
  ...
  const syntaxProbe = spawnSync(
    ...
    ["//B", "//NoLogo", path.join(root, "src", "runtime", "CodexBot-Hidden-Runner.vbs"), "invalid-mode"],
    { windowsHide: true },
  );
...
test("launcher records failures even though it has no console", () => {
  ...
  assert.match(launcher, /trap \{[\s\S]*Write-SafeLauncherLog \("Launch failed: "/);
...
test("both supervisors safely adopt a verified same-install listener that won a bind race", () => {
  for (const relativePath of ["src/runtime/Launch-Codex-Bot.ps1", "src/runtime/CodexBot-Watchdog.ps1"]) {
    ...
    assert.match(source, /\$processIds = @\(Get-NetTCPConnection[\s\S]*OwningProcess -Unique\)/);
    assert.match(source, /Test-ExpectedLoopbackListener -Port \$Port -ExpectedExecutable \$ExpectedExecutable -ExpectedProcessId \$candidateProcessId/);
    assert.match(source, /\$script:expectedProxyProcessId = \$verifiedProcessId/);
    assert.match(source, /\$script:expectedHostProcessId = \$verifiedProcessId/);
