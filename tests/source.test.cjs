...
test("release inputs are explicit, third-party bits are pinned, and uninstall offers a data wipe", () => {
  ...
  assert.match(manifest, /#define AppCanonicalVersion "0\.1\.8"/);
  assert.match(
    manifest,
  ...
  assert.match(manifest, /#define AppVersion "0\.1\.8 DEVELOPMENT TEST BUILD"/);
  assert.match(
    manifest,
  ...
  assert.match(
    manifest,
    /#define AppOutputBaseFilename "OpenBot-Setup-0\.1\.8"/,
  );
