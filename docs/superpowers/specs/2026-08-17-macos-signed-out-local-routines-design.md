...
The store consumes a narrow macOS-only `local-automation-native-io.cjs` state-I/O adapter. The adapter requires the already-existing private state directory, opens it with `O_DIRECTORY | O_NOFOLLOW`, captures its exact identity, and verifies the matching macOS file-ID path `/.vol/<device>/<inode>`. Every state-file lookup, bounded read, sibling temporary-file creation, file sync, atomic replacement, exact read-back, cleanup, and directory sync uses only that stable parent while the directory handle remains open; the configured pathname is re-canonicalized and identity-checked at operation boundaries. Unsupported VolFS mounts, symlinked or substituted ancestors, foreign-owned paths, non-`0700` roots, and non-`0600` or multiply linked files fail closed without repair. The adapter launches no child process and mutates no native helper. The JSON store itself receives only `stateIO.read()` and `stateIO.write(source)`, which keeps filesystem fault injection separate from record validation and mutation semantics.
